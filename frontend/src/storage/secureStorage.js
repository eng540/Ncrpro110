...
class SecureStorage {
  ...
  async saveLatrine(data) {
    ...
    const envelope = {
      ...
      beneficiary_hh: data.beneficiary_hh ? (await cryptoService.encryptField(data.beneficiary_hh, { entity: 'latrine', field: 'beneficiary_hh', rid: safeId })).toDB() : null,
      gps_coordinates: data.gps_coordinates ? (await cryptoService.encryptField(data.gps_coordinates, { entity: 'latrine', field: 'gps_coordinates', rid: safeId })).toDB() : null,
      site_engineer: data.site_engineer ? (await cryptoService.encryptField(data.site_engineer, { entity: 'latrine', field: 'site_engineer', rid: safeId })).toDB() : null,
    };
  ...
  async saveRemark(data) {
    ...
    // 🌟 إصلاح: تمرير safeUuid كنص لدالة التشفير
    if (data.description) {
  ...
  async pushSyncQueue(type, data) {
    ...
    if (type === 'CREATE_REMARK' || type === 'UPDATE_REMARK') {
      if (secureData.description) {
        ...
        secureData.description = JSON.stringify(env.toDB()._env);
      }
