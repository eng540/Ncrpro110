...
const BulkUpdate = ({ onOpenRemarks }) => {
  ...
  const handleSingleUpdate = useCallback(async (item, latrineId) => {
    ...
    try {
      ...
      if (isNaN(qty) || qty < 0 || qty > item.planned_qty) {
        alert(`الكمية غير صالحة. الحد الأقصى لهذا البند: ${item.planned_qty}`);
        return;
      }
  ...
  const handleApplyBulkUpdate = async () => {
    ...
    if (bulkQty === '' || isNaN(parseFloat(bulkQty))) {
      alert('يرجى إدخال الكمية المنفذة.');
      return;
    }
      ...
      await db.transaction('rw', db.boq_items, db.sync_queue, async () => {
        for (const item of itemsToProcess) {
          if (qty > item.planned_qty) {
            console.warn(`تخطي ${item.id}: الكمية ${qty} تتجاوز المخطط ${item.planned_qty}`);
            continue;
          }
          ...
          await db.boq_items.update(item.id, { 
            achieved_qty: qty, status: bulkStatus, quality_pass: bulkQuality 
          });
          await pushToSyncQueue('UPDATE_BOQ', { 
  ...
  return (
    <div style={{ direction: 'rtl' }}>
      <h2 style={{ color: '#1F4E78', marginBottom: '20px' }}>
        إدارة البند {selectedBoqCode} في جميع الحمامات
      </h2>
      ...
      <div style={{ background: '#e8f4f8', border: '1px solid #bdc3c7', padding: '15px', borderRadius: '8px', marginBottom: '20px' }}>
        ...
        <div style={{ display: 'flex', gap: '15px', flexWrap: 'wrap', alignItems: 'flex-end' }}>
          ...
          <button 
            ...
            disabled={isSaving || selectedLatrines.length === 0} 
            style={{ padding: '10px 20px', background: selectedLatrines.length > 0 ? '#16a085' : '#bdc3c7', color: 'white', border: 'none', borderRadius: '4px', cursor: selectedLatrines.length > 0 ? 'pointer' : 'not-allowed', fontWeight: 'bold' }}
          >
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', minWidth: '1100px' }}>
          <thead style={{ background: '#1F4E78', color: 'white' }}>
            <tr>
              ...
              <th style={{ padding: '12px', textAlign: 'right' }}>الحمام</th>
              <th style={{ padding: '12px', textAlign: 'right' }}>المربع</th>
              <th style={{ padding: '12px', textAlign: 'center' }}>المخطط ({targetBoqInfo?.name})</th>
          ...
          <tbody>
            {tableData.length === 0 ? (
              <tr><td colSpan="9" style={{ padding: '20px', textAlign: 'center' }}>لا توجد بيانات محلية. يرجى المزامنة.</td></tr>
            ) : (
              tableData.map(row => (
                <tr key={row.id} style={{ borderBottom: '1px solid #eee', backgroundColor: selectedLatrines.includes(row.id) ? '#f1f8ff' : 'transparent' }}>
                  ...
                  <td style={{ padding: '12px', color: '#7f8c8d' }}>{row.block_no}</td>
                  <td style={{ padding: '12px', textAlign: 'center', fontWeight: 'bold' }}>{row.boqItem.planned_qty} {row.boqItem.unit}</td>
                  <td style={{ padding: '12px', fontWeight: 'bold', color: '#7f8c8d' }}>{row.boqItem.achieved_qty} / {row.boqItem.planned_qty}</td>
                  ...
                  <td style={{ padding: '12px' }}>
                    <select id={`bulk-status-${row.boqItem.id}`} defaultValue={row.boqItem.status || 'not_started'} style={{ padding: '6px', borderRadius: '4px', border: '1px solid #ccc', fontSize: '12px' }}>
                      {itemStatuses.map(s => <option key={s.value} value={s.value}>{s.label}</option>)}
                    </select>
                  </td>
                  <td style={{ padding: '12px' }}>
                    <select 
                      id={`bulk-quality-${row.boqItem.id}`} defaultValue={row.boqItem.quality_pass || 'pending'} 
                      style={{ padding: '6px', borderRadius: '4px', border: '1px solid #ccc', fontSize: '12px', backgroundColor: qualityColors[row.boqItem.quality_pass || 'pending'] }}
                  ...
                  <td style={{ padding: '12px', textAlign: 'center' }}>
                    <div style={{ display: 'flex', gap: '5px', justifyContent: 'center', flexDirection: 'column' }}>
                      ...
                      <button onClick={() => onOpenRemarks?.(row.id, selectedBoqCode)} style={{ padding: '4px 8px', background: '#e74c3c', color: 'white', border: 'none', borderRadius: '4px', cursor: 'pointer', fontSize: '11px' }}>+ ملاحظة</button>
                    </div>
                  </td>
                </tr>
              ))
