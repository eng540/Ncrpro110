// DailyLogList.js – عرض سجل التقارير اليومية
  ...
  const fetchLogs = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(`فشل جلب البيانات: ${res.status} ${errorText}`);
      }
      const data = await res.json();
      setLogs(data);
    } catch (err) {
      console.error('DailyLogList fetch error:', err);
            ...
            >
              ...
              >
                <div style={{ textAlign: 'center' }}>
                  <div
                    style={{ fontSize: '22px', fontWeight: 'bold', color: '#1F4E78' }}
                  >
                    {log.latrines_inspected}
                  </div>
                ...
                <div style={{ textAlign: 'center' }}>
                  ...
                  >
                    {log.latrines_accepted}
                  </div>
                  <div style={{ fontSize: '12px', color: '#666' }}>✅ حمامات مُقبولة</div>
                </div>
                <div style={{ textAlign: 'center' }}>
                  <div
                    style={{ fontSize: '22px', fontWeight: 'bold', color: '#e74c3c' }}
                  >
                    {log.remarks_issued}
                  </div>
                ...
                <div style={{ textAlign: 'center' }}>
                  <div
                    style={{ fontSize: '22px', fontWeight: 'bold', color: '#f39c12' }}
                  >
                    {log.manpower || 0}
                  </div>
                ...
                <div style={{ textAlign: 'center' }}>
                  <div
                    style={{ fontSize: '22px', fontWeight: 'bold', color: '#9b59b6' }}
                  >
                    {log.weather || '—'}
                  </div>
